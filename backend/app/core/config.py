...
class Settings:
    ...
    FRESHGUARD_VISION_MODEL: str = os.getenv("FRESHGUARD_VISION_MODEL", "v5").lower()
    CORS_ORIGINS: list = [origin.strip() for origin in os.getenv("CORS_ORIGINS", "https://fresh-guard-ai-delta.vercel.app,https://freshguard-ai.vercel.app,http://localhost:3000,http://127.0.0.1:3000,http://localhost:8000,http://127.0.0.1:8000,*").split(",") if origin.strip()]
